...
public class Controller {
   ...
   /*public static void main(String[] args) throws Exception {
        Database.connect();
        Controller controller = new Controller(20000, 100, 20, 3, 5, 4, 4, 4);
        eventList.add(controller.entry.moveQueue(random.nextBoolean()));
        controller.run();
        System.out.println("Simulaatio on päättynyt.");
        int kävijätYhteensä = controller.gaKävijämäärä + controller.vipKävijämäärä;
        if (Customer.getPääsiSaliin() == kävijätYhteensä) {
            ...
            System.out.println("Saliin pääsi " + Customer.getPääsiSaliin() + " / " + kävijätYhteensä);
        }
